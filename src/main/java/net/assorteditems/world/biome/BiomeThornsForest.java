...
import net.mcreator.assorteditems.block.BlockThorns;
import net.mcreator.assorteditems.ElementsAssortedItemsMod;

import java.util.Random;

@ElementsAssortedItemsMod.ModElement.Tag
public class BiomeThornsForest extends ElementsAssortedItemsMod.ModElement {
	@GameRegistry.ObjectHolder("assorted_items:thorns_forest")
	public static final BiomeGenCustom biome = null;
	public BiomeThornsForest(ElementsAssortedItemsMod instance) {
		super(instance, 343);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("thorns_forest").setRainfall(0.7F).setBaseHeight(0.1F).setHeightVariation(0.2F).setTemperature(0.4F));
			setRegistryName("thorns_forest");
	...
	static class CustomTree extends WorldGenAbstractTree {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												|| state.getBlock() == BlockThorns.block.getDefaultState().getBlock()
												|| state.getBlock() == Blocks.LOG.getStateFromMeta(0).getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, Blocks.LOG.getStateFromMeta(0));
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == BlockThorns.block.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.LOG.getStateFromMeta(0).getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), Blocks.LOG.getStateFromMeta(0));
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										this.setBlockAndNotifyAdequately(world, position.add(-1, genh, 0), BlockThorns.block.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										this.setBlockAndNotifyAdequately(world, position.add(1, genh, 0), BlockThorns.block.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, -1)))
										this.setBlockAndNotifyAdequately(world, position.add(0, genh, -1), BlockThorns.block.getDefaultState());
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, 1)))
										this.setBlockAndNotifyAdequately(world, position.add(0, genh, 1), BlockThorns.block.getDefaultState());
								}
							}
		...
		private void addVines(World world, BlockPos pos) {
			this.setBlockAndNotifyAdequately(world, pos, BlockThorns.block.getDefaultState());
			int i = 5;
			for (BlockPos blockpos = pos.down(); world.isAirBlock(blockpos) && i > 0; --i) {
				this.setBlockAndNotifyAdequately(world, blockpos, BlockThorns.block.getDefaultState());
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.LOG.getStateFromMeta(0).getBlock()
					|| blockType == Blocks.LOG.getStateFromMeta(0).getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
